...
import { Menu, X, GraduationCap, BookOpen, Users, Award, Calendar, ArrowRight, CheckCircle, Star, Quote, MapPin, Phone, Mail, Clock, Facebook, Instagram, Youtube } from 'lucide-react';
...
const IMAGES = {
  ...
  chess: ['https://d64gsuwffb70l.cloudfront.net/6960c785dbbe68a330bb6214_1767950512839_a4125a0a.jpg'],
  graduation: ['https://d64gsuwffb70l.cloudfront.net/6960c785dbbe68a330bb6214_1767950546969_34e2e0d5.jpg'],
};
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} className="text-white hover:text-[#C9A24D] transition-colors font-medium">
                {link.name}
              </a>
            ))}
            <a href="#programs" className="bg-[#C9A24D] text-white px-6 py-2 rounded-lg font-medium hover:bg-[#b8923f] transition-colors">
              Enroll Now
            </a>
          </div>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden pb-4">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} onClick={() => setMobileMenuOpen(false)} className="block text-white hover:text-[#C9A24D] py-2">
                {link.name}
              </a>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div>
            ...
            <p className="text-gray-400 text-sm">Building academic excellence since 2001.</p>
          </div>
          <div>
            <h4 className="font-bold text-[#C9A24D] mb-4">Quick Links</h4>
          ...
          <div>
            <h4 className="font-bold text-[#C9A24D] mb-4">Programs</h4>
            <ul className="space-y-2 text-sm text-gray-400">
          ...
          <div>
            <h4 className="font-bold text-[#C9A24D] mb-4">Contact</h4>
            <div className="space-y-2 text-sm text-gray-400">
              <p className="flex items-center"><MapPin className="w-4 h-4 mr-2 text-[#C9A24D]" />Male', Maldives</p>
              <p className="flex items-center"><Phone className="w-4 h-4 mr-2 text-[#C9A24D]" />+960 XXX XXXX</p>
              <p className="flex items-center"><Mail className="w-4 h-4 mr-2 text-[#C9A24D]" />info@elc.mv</p>
            </div>
          </div>
        </div>
...
function CourseCard({ course }: { course: Course }) {
  return (
    <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all p-6">
      <span className="text-xs font-medium text-[#1F6F43] bg-[#1F6F43]/10 px-3 py-1 rounded-full">{course.category}</span>
      ...
      <div className="flex items-center justify-between pt-4 border-t">
        ...
        <a href="#contact" className="text-[#1F6F43] font-medium hover:text-[#C9A24D] flex items-center">
      ...
      <main className="flex-grow">
        ...
        <section id="home" className="relative bg-[#1F6F43] overflow-hidden">
          <div className="absolute inset-0">
            <img src={IMAGES.hero} alt="Students" className="w-full h-full object-cover opacity-20" />
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <p className="text-white/90 text-lg md:text-xl mb-8">
                  Everyone's Learning Centre empowers students in the Maldives with quality English education. From ages 4 to 16, we prepare students for success.
                </p>
                <div className="flex flex-col sm:flex-row gap-4">
                  <a href="#programs" className="inline-flex items-center justify-center bg-[#C9A24D] text-white px-8 py-4 rounded-lg font-semibold hover:bg-[#b8923f] transition-colors text-lg">
                    View Programs <ArrowRight className="w-5 h-5 ml-2" />
                  </a>
                  <a href="#contact" className="inline-flex items-center justify-center bg-white text-[#1F6F43] px-8 py-4 rounded-lg font-semibold hover:bg-gray-100 transition-colors text-lg">
                    Enroll / Apply
                  </a>
                </div>
              </div>
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <p className="text-gray-600 mb-6">
                  Our mission is to strengthen English proficiency for students in English-medium schools and adult learners. We have reached tens of thousands of students across the Maldives.
                </p>
                <div className="space-y-4">
        ...
        <section id="programs" className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="font-serif text-3xl md:text-4xl font-bold text-[#2B2B2B] mb-4">Our Programs</h2>
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-8 max-w-md mx-auto">
              <div className="space-y-4 text-white text-left">
                <p className="flex items-center"><Phone className="w-5 h-5 mr-3 text-[#C9A24D]" />+960 XXX XXXX</p>
                <p className="flex items-center"><Mail className="w-5 h-5 mr-3 text-[#C9A24D]" />info@elc.mv</p>
                <p className="flex items-center"><MapPin className="w-5 h-5 mr-3 text-[#C9A24D]" />Male', Maldives</p>
                <p className="flex items-center"><Clock className="w-5 h-5 mr-3 text-[#C9A24D]" />Sat-Thu: 9AM-9PM</p>
              </div>
            </div>
          </div>
        </section>
