...
export default function Apply() {
  ...
  // Pricing: try to read from selectedCourse.selected_price (set in CourseDetail)
  ...
  useEffect(() => {
    if (courseId && !selectedCourse) {
      ...
      // Pull selected pricing from context if present
      const sp = (selectedCourse as any)?.selected_price ?? null;
      if (sp) setSelectedPricing(sp);
    } else if (!courseId) {
      navigate('/programs');
    }
  ...
  useEffect(() => {
    // If we have a course but no selected pricing, fetch a default pricing option
    if (course?.id && !selectedPricing) {
      fetchDefaultPricing(course.id);
    }
  ...
  const goToPayment = () => {
    ...
    // Store payment context for MakePayment page
    ...
    // Store application form data to be used later after payment approval
    localStorage.setItem(
      'pendingApplication',
      JSON.stringify({
        ...formData,
    ...
    // Also keep in app context (optional)
    setApplicationData({
      ...(formData as Application),
      course_id: course.id
    } as Application);

    // If user not logged in, send to login and then come back to make-payment
    if (!user) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // New flow: do NOT insert into applications here.
    // Just save pending data + go to make-payment.
    goToPayment();
  };
  ...
  const displayAmount =
    selectedPricing?.amount ?? (course.fee ?? null);

  const displayLabel =
    selectedPricing?.label || (selectedPricing?.billing_period ? `${selectedPricing.billing_period} Fee` : 'Course Fee');
              ...
              <div className="bg-[#F4F6F8] rounded-lg p-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Payment Option: <span className="font-medium">{displayLabel}</span>
                    </p>
                  </div>
